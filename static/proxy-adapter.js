...
(function () {
  ...
  const ingressPath = isIngress
    ? window.location.pathname.split("/api/hassio_ingress/")[1].split("/")[0]
    : "";
  ...
  XMLHttpRequest.prototype.open = function (
    ...
    if (url && (url.includes(".jsn") || url.includes("/api/"))) {
      ...
      console.log(
        "MyPV Proxy: Intercepted API call:",
        url,
        "-> Proxied to:",
        this._proxiedUrl
      );
  ...
  window.fetch = function (url, options) {
    if (
      typeof url === "string" &&
      (url.includes(".jsn") || url.includes("/api/"))
    ) {
      const proxiedUrl = proxyUrl(url);

      console.log(
        "MyPV Proxy: Intercepted fetch call:",
        url,
        "-> Proxied to:",
        proxiedUrl
      );
    ...
    $.ajax = function (settings) {
      if (typeof settings === "string") {
        ...
        if (url.includes(".jsn") || url.includes("/api/")) {
          ...
          console.log(
            "MyPV Proxy: Intercepted jQuery AJAX call:",
            url,
            "-> Proxied to:",
            arguments[0]
          );
